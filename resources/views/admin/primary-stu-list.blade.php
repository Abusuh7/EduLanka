                                ...
                                @foreach ($primaryStudents as $primaryStudent)
                                    <tr>
                                        ...
                                        <td class="px-6 py-4 whitespace-no-wrap text-right text-sm leading-5 font-medium">
                                            <a href="{{ route('studentProfile', $primaryStudent->id) }}" class="text-indigo-600 hover:text-indigo-900">View Profile</a>
                                            <a href="{{ route('editStudentProfile', $primaryStudent->id) }}" class="ml-2 text-indigo-600 hover:text-indigo-900">Edit</a>
                                            @if ($primaryStudent->status === 'deactivated')
                                                <a href="{{ route('activateStudent', $primaryStudent->id) }}" class="ml-2 text-green-600 hover:text-green-900">Activate</a>
                                            @else
                                                <a href="{{ route('deactivateStudent', $primaryStudent->id) }}" class="ml-2 text-red-600 hover:text-red-900">Deactivate</a>
                                            @endif
